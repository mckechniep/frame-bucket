...
- **Line-height**: Body 1.55–1.7 (generous, letting the eye flow through long passages). Display **1.1–1.15** (tight, but with descender clearance). Tight tracking and tight leading are different axes — Editorial's signature is the former, not the latter. **Floor of 1.1 prevents descender-into-next-line collisions**: when a headline contains a descender (g, j, p, q, y) at the end of one line and the following line starts with an ascender (b, d, f, h, k, l, t) or a capital, line-height ≤ 1.0 lets the strokes visually crash. Letterforms _within_ a line can touch (that's signature-Editorial); letterforms _between_ lines should not. If a specific headline's letterform combination is verifiably safe, 1.05 is permissible — but 1.1 is the safe default for one-shot generation.
