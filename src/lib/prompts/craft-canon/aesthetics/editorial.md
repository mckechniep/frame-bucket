...
- **Side-captions require sibling space.** The Editorial side-caption move (figure spans 8 cols, caption sits in the remaining 4) works _only when_ the figure is genuinely sharing horizontal space with another element. **When a figure stands alone** — a centered hero photo, a full-bleed image without adjacent text, a single-column section with a single image — **the caption stacks below the figure, period.** Synthesizing negative space _just_ to put a caption beside an otherwise full-width figure produces awkward orphaned columns. The pattern works when the page is _already_ a multi-column composition; it doesn't manufacture composition where there isn't one. (→ source: ui-design:layout-grid, designer-toolkit:ux-writing)
- **Multi-column hero (display headline + intro paragraph) requires balanced column heights.** Editorial allows splitting a hero into a display headline in one column and an intro/standfirst paragraph in another — it's a defining magazine move. **It only works when both columns reach similar visual weight at desktop widths**: typically a 3–5 line headline matched against 3–5 lines of body, both occupying roughly the same vertical extent. **It fails when the columns diverge**: a 3-line headline placed next to a 7+ line intro paragraph orphans the headline above generic dead space, and a long intro placed next to a short headline reads as space-filling rather than composition. The discipline: at 1024px+, _measure_ — if either column would render more than ~1.5× the other's height, either shorten the intro to balance, widen the headline column to absorb more lines, or stack vertically. Side-by-side hero is a deliberate composition; orphaned columns are a layout collision. (→ source: ui-design:layout-grid, designer-toolkit:ux-writing)
- **Feature sections with image + text-block follow the same height-ratio rule.** A feature region pairing a portrait image (typically tall) with a 2–3 sentence text block (typically short) at desktop widths produces the same orphaned-column failure as the hero case above — except more severe, because feature images are often taller than hero headlines. **Default for image + short-text features at wide breakpoints: stack vertically** with the image constrained to `max-width: 60ch` or similar, text below. Side-by-side is permissible only when (a) the text block is genuinely substantive (5+ lines at the column width), or (b) the image is capped with `max-height` to match the text column's rendered height. The common error — a tall portrait image in column 1 next to a 3-sentence paragraph in column 2 with `align-items: start` and ample dead space below — is a wide-viewport reflow failure, not Editorial signature. (→ source: ui-design:layout-grid, ui-design:spacing-system)
- **Staggered image+text pairs share vertical level, they do not stack with dead space.** Editorial's iconic move is a series of image+text pairs offset vertically across a multi-column grid — image at columns 1–4, text at columns 6–9, each starting at slightly different rows so they read as deliberately interleaved. **The discipline: text and image must overlap vertically by at least one row, not stack as separate rectangles with empty side-space.** Concrete failure mode: a bell image at `grid-row: 4 / 7` in cols 1–4 (rows 4–6) followed by a body-text block at `grid-row: 7 / 10` in cols 5–8 (rows 7–9). Each occupies its own vertical band; the image has dead space to its right (cols 5–8 of rows 4–6), and the text has dead space to its left (cols 1–4 of rows 7–9). The composition reads as two stacked things, not as a staggered composition. **The fix: text starts before the image ends.** If the image spans rows 4–6, the text should start at row 5 (overlapping the image's row 5 and 6) — text rows 5–7, image rows 4–6 = one row of vertical overlap. The page reads as a single woven editorial rhythm rather than a list of rectangles. **Across a section with multiple image+text pairs, every pair should share at least one row of vertical overlap with its partner.** A page where pair 1 overlaps cleanly but pair 2 stacks (the bell-and-II.-text failure mode) reads as inconsistent composition — fix the inconsistency by giving every pair the same staggered overlap discipline. (→ source: ui-design:layout-grid)
