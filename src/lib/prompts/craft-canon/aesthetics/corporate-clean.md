...
- **Wide-viewport reflows are functional, not formulaic.** Corporate Clean's instinct is to "use horizontal space efficiently" — that's correct _only when the content forms a natural row_. Side-by-side at wide widths is correct for: **content comparisons** (feature cards, pricing tiers, before/after panels), **trust-signal layouts** (customer-logo strip, integration logos, security/compliance badges), **balanced testimonial pairs**. Side-by-side is _wrong_ for: **figure + caption**, **hero text + supporting info**, **body copy + secondary block of mismatched density**. The discipline: ask "does this content form a natural row of equal items, or a natural stack of sequential blocks?" — if stack, cap container `max-width` and center; don't artificially split. (→ source: ui-design:layout-grid)
- **Captions stack below figures** at every viewport. Corporate Clean's predictability extends to image-caption pairings: the caption belongs predictably under the photo. Wide widths cap the figure's `max-width` and keep the caption stacked below; never split into adjacent grid columns.
