...
- **Maximum three font families.** Two is often enough — a display face and a body face — but a third is permitted when it earns a specific role: monospace for technical aesthetics or data-heavy layouts, an italic-only or script display for pull-quotes and marginalia in editorial work, or a tabular face for figure callouts. Never use four; that signals a missed pairing decision rather than a deliberate one. (→ source: rules/web/performance.md)
- Pair display + body deliberately; no two-sans defaults unless the aesthetic is Swiss/grotesk. (→ source: ui-design:type-system)
- **Google Fonts variable-axis URL syntax: every value tuple must match the declared axis count.** When requesting variable fonts with multiple axes (e.g., Fraunces with `opsz,wght,SOFT`, Recursive with `slnt,CRSV,MONO,CASL,wght`), the v2 API parses `family=Name:axis1,axis2,axis3@val1,val2,val3;val1,val2,val3` — **each `;`-separated tuple after `@` must contain exactly as many comma-separated values as axes declared.** Mismatched counts (3 axes declared, 2 values per tuple) return an HTML 400 error page rather than CSS, which browsers reject due to MIME type (`text/html` mismatch with `nosniff`) — the font silently fails to load and the page falls back to system serifs/sans, catastrophically degrading the typography. The model has no rendering loop to catch this. **Pick the simplest URL that loads the weights actually used.** When in doubt, drop optional axes (`SOFT`, `WONK`, `GRAD`, `MONO`) and use the canonical 2-axis form: `family=Fraunces:opsz,wght@9..144,400;9..144,500;9..144,700`. If a 3-axis URL is genuinely needed, every tuple must supply a value for every axis: `family=Fraunces:opsz,wght,SOFT@9..144,400,0;9..144,500,0;9..144,700,0`. (→ source: rules/web/performance.md)
